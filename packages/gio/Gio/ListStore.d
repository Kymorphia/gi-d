...
class ListStore : ObjectG, ListModel
{
  ...
  bool findWithEqualFuncFull(ObjectG item, EqualFuncFull equalFunc, out uint position)
  {
    ...
    _static_equalFunc = equalFunc;
    bool _retval;
    auto _equalFunc = freezeDelegate(cast(void*)&equalFunc);
    _retval = g_list_store_find_with_equal_func_full(cast(GListStore*)cPtr, item ? cast(ObjectC*)item.cPtr(false) : null, &_equalFuncCallback, _equalFunc, cast(uint*)&position);
    _static_equalFunc = null;
    return _retval;
  }
  ...
  uint insertSorted(ObjectG item, CompareDataFunc compareFunc)
  {
    ...
    _retval = g_list_store_insert_sorted(cast(GListStore*)cPtr, item ? cast(ObjectC*)item.cPtr(false) : null, &_compareFuncCallback, _compareFunc);
  ...
  void sort(CompareDataFunc compareFunc)
  {
    ...
    _static_compareFunc = compareFunc;
    auto _compareFunc = freezeDelegate(cast(void*)&compareFunc);
    g_list_store_sort(cast(GListStore*)cPtr, &_compareFuncCallback, _compareFunc);
    _static_compareFunc = null;
  }
